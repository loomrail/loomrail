-- Q16 provider allowance is advisory account telemetry, stored separately from Loomrail's hard
-- budget. One normalized latest snapshot is kept per Project/provider; the append-only Event in
-- the same transaction preserves every accepted observation.

CREATE TABLE provider_allowance_snapshots (
...
ALTER TABLE events RENAME TO events_v35;
