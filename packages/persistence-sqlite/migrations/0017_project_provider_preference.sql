-- Provider Selection (`docs/plans/31-provider-selection-auto-detection-spec.ru.md`). Existing and
-- new Projects start in AUTO. This is durable execution configuration, not browser-local UI state:
-- the daemon must make the same routing decision after restart and before any browser reconnects.
ALTER TABLE projects ADD COLUMN provider_preference TEXT NOT NULL DEFAULT 'AUTO'
  CHECK (provider_preference IN ('AUTO', 'CODEX', 'CLAUDE_CODE', 'MOCK'));

-- `events.type` is closed. Rebuild it with the preference audit fact and preserve every sequence
-- and cursor exactly; Events have no inbound foreign keys.
