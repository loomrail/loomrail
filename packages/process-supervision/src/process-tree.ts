...
const PROCESS_START_QUERY_TIMEOUT_MS = 10_000;
...
const reapWindowsDescendants = async (
  ...
  const script = [
    ...
    "if ($null -eq $candidate.CreationDate) { throw 'Process identity unavailable'; }",
    // Windows retains a creator PID after that creator exits. A process older than the root cannot
    // belong to the tree this supervisor started, even when its stale parent PID now equals rootPid.
    "if ($candidate.CreationDate.ToUniversalTime() -lt $minimumCreation) { continue; }",
    ...
    "$quietScans = 0;",
    "$observationFailures++;",
    "if ($observationFailures -ge 3) { throw; }",
