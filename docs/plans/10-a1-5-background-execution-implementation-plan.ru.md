...
> **Разошлось с реализацией (финальное ревью).** `onStatus` и `EventChannelStatus` не дожили до
> сдачи. Признак состояния канала протаскивался через контекст рабочей области и не отображался
> нигде — «провод, к которому ничего не подключено с обеих сторон». Отобразить его было нельзя, не
> тронув `AppFrame.tsx` и `styles.css`, которые в тот момент держала параллельная сессия, поэтому
> поле удалено. Решение о мёртвом канале живёт в `scopesForChannelStatus` и доезжает до владельца
> через уже существующую панель восстановления. Ниже — то, что планировалось; истина о собранном
> находится в коде.
